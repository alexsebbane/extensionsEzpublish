...
{* NEW!!! IMPERSONATE BUTTON. Redirects to main siteaccess/simulate/impersonate *}
{def $ok=fetch( 'user',     'has_access_to',
          hash( 'module',   'simulate',
                'function', 'impersonate'))
     $mainsiteaccess=ezini('SiteSettings', 'DefaultAccess',,,true())
     $mainsiteurl=ezini('SiteSettings', 'SiteURL')}
{set $ok=and($ok,ne($node.contentobject_id,ezini('UserSettings', 'AnonymousUserID')))}
{set $ok=true()}
<input class="button" type="button" name="ActionImpersonate" value="{'Impersonate'|i18n('extension/ast_simulate_user')}" onclick="window.location='{concat('http://', $mainsiteurl, '/simulate/impersonate/', $node.contentobject_id)|ezurl('no')|wash('javascript')}';" {if eq($ok,false())}disabled="disabled" {/if}/>
{undef $ok $mainsiteaccess $mainsiteurl}
